...
const dataFetcher = async (item, category, startPage = 1) => {
  try {
      ...
      for (const course of courseLinks) {
        ...
        while (retries < maxRetries) {
          try {
            ...
            let existingData = [];
            const fileName = `${category}_${item}.json`;

            try {
              const rawData = fs.readFileSync(`output/${fileName}`);
            ...
            fs.writeFileSync(`output/${fileName}`, JSON.stringify(existingData, null, 2));
